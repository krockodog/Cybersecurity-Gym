...
interface IRStep {
  id: string; phase: string; icon: React.ReactNode;
  situation: string; alert: string;
...
const INITIAL_STEPS: IRStep[] = [
  { id: 'preparation', phase: 'Preparation', icon: <BookOpen size={18} />,
    situation: 'Your organization has no formal IR plan. The CISO asks you to prioritize one preparation activity before any incidents occur.',
    ...
    correctIndex: 1,
    explanation: 'NIST SP 800-61 emphasizes a documented IR plan with policies, procedures, and communication plans as the foundation of preparation.',
  ...
  { id: 'detection', phase: 'Detection & Analysis', icon: <Search size={18} />,
    ...
    alert: 'ALERT: Brute-force on DC01. Source: 10.0.1.47. 847 failed attempts then success at 03:14 UTC.',
    ...
    correctIndex: 1,
    explanation: 'Detection requires correlating alerts across sources, verifying the incident is real (not a false positive), and classifying severity before containment.',
    selectedIndex: null, completed: false },
  { id: 'containment', phase: 'Containment', icon: <Box size={18} />,
    situation: 'Confirmed attack. The compromised workstation (10.0.1.47) is exfiltrating data to an external C2 server. The attacker has domain admin credentials.',
    alert: 'CONFIRMED: Active exfiltration to 185.220.101.33:443. Lateral movement detected to FILE-SRV01 and DB-SRV02.',
    options: ['Wipe all compromised systems immediately', 'Isolate systems, capture forensic images, and reset compromised credentials', 'Shut down the entire corporate network', 'Contact the attacker to negotiate stopping the breach'],
    correctIndex: 1,
    explanation: 'Containment means isolating affected systems while preserving evidence. Forensic images must be captured before any remediation begins.',
    selectedIndex: null, completed: false },
  { id: 'eradication', phase: 'Eradication', icon: <Trash2 size={18} />,
    ...
    alert: 'FORENSICS: Rootkit (Diamorphine) on WKS-047. Backdoor "WinUpdate_Svc" on DC01. Registry persistence confirmed.',
    ...
    correctIndex: 1,
    explanation: 'Rootkits cannot be reliably cleaned. Rebuild from known-good images, remove all persistence mechanisms, and patch the entry vector.',
    selectedIndex: null, completed: false },
  { id: 'recovery', phase: 'Recovery', icon: <RefreshCcw size={18} />,
    situation: 'Systems rebuilt, accounts reset, vulnerability patched. Backup integrity verified. Time to bring systems back online safely.',
    alert: 'STATUS: 4 systems rebuilt, 12 accounts reset, vulnerability patched. Backup integrity verified. Ready for recovery.',
    options: ['Bring all systems online simultaneously', 'Restore in phases with enhanced monitoring, validate each before proceeding', 'Keep all systems offline for 30 days to be safe', 'Restore only the DC and let users rebuild their workstations'],
    correctIndex: 1,
    explanation: 'Phased recovery with validation and enhanced monitoring detects any remaining compromise before expanding access.',
    selectedIndex: null, completed: false },
  { id: 'lessons', phase: 'Lessons Learned', icon: <BookOpen size={18} />,
    situation: 'Incident resolved after 72 hours. 4 systems affected, 15 accounts compromised, ~2GB data accessed. Management wants to prevent recurrence.',
    alert: 'RESOLVED: Incident closed. Total impact: 4 systems, 15 accounts, ~2GB data accessed. No confirmed data loss.',
    ...
    correctIndex: 2,
    explanation: 'Lessons Learned requires a formal review with all stakeholders, documentation, IR plan updates, and implementing controls to prevent recurrence.',
    selectedIndex: null, completed: false },
];
...
export default function IncidentResponsePBQ({ onComplete }: Props) {
  ...
  const threatLevel = currentStep === 0 && !steps[0].completed
    ? 'low' as const
    : threatLevels[Math.min(currentStep, threatLevels.length - 1)];
  ...
  const selectOption = useCallback((optionIndex: number) => {
    ...
    if (isCorrect) {
      ...
      if (currentStep === steps.length - 1) {
        ...
        setTimeout(() => { setCurrentStep(prev => prev + 1); setFeedback(null); }, 1600);
      }
    } else {
      setFeedback('wrong');
      setTimeout(() => {
  ...
  return (
    <div className="w-full">
      ...
      {/* Threat Level Banner */}
      <motion.div
        animate={{ borderColor: threatColors[threatLevel], boxShadow: `0 0 15px ${threatColors[threatLevel]}30` }}
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className={`text-xs font-mono ${isOk ? 'text-[#00ff41]' : isBad ? 'text-[#ff3366]' : 'text-[#7da0c4]'}`}>
            ...
            <AnimatePresence>
              ...
              {feedback === 'wrong' && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  ...
                  <p className="text-sm text-[#ff3366] font-display">Not the best response. Try again.</p>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
                ...
                <div className="flex-1">
                  ...
                  {step.completed && (
                    <p className="text-[10px] text-[#7da0c4] mt-0.5">
                      {step.options[step.correctIndex].substring(0, 50)}...
                    </p>
                  )}
