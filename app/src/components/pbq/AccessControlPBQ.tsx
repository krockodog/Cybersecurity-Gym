...
const INITIAL_SCENARIOS: Scenario[] = [
  { id: 'rbac', title: 'RBAC Assignment', icon: <Users size={18} />,
    ...
    correctIndex: 2,
    explanation: 'Finance_Clerk matches exactly: AP, expenses, and read-only GL. Finance_Analyst adds unnecessary payroll read access.',
  ...
  { id: 'least-privilege', title: 'Least Privilege Violation', icon: <Eye size={18} />,
    ...
    correctIndex: 1,
    explanation: 'Developers need dev databases but not prod write access. Production changes require change management with separate credentials.',
    selectedIndex: null, completed: false },
  { id: 'mfa', title: 'MFA Configuration', icon: <Key size={18} />,
    ...
    correctIndex: 1,
    explanation: 'Zero-trust requires MFA everywhere with stronger factors (hardware tokens) for higher risk. SMS is weak (SIM-swapping).',
    selectedIndex: null, completed: false },
  { id: 'separation', title: 'Separation of Duties', icon: <UserCheck size={18} />,
    ...
    correctIndex: 2,
    explanation: 'Same person creating PO and approving payment is a classic SoD violation. Ordering and receipt/payment authorization must be separate.',
    selectedIndex: null, completed: false },
  { id: 'audit', title: 'Access Log Audit', icon: <ClipboardList size={18} />,
    ...
    correctIndex: 2,
    explanation: 'Bulk export of ~3,000 customer records at 1:30 AM is a strong data exfiltration indicator requiring immediate investigation.',
    selectedIndex: null, completed: false },
];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[58%_42%] gap-4">
          ...
          >
            ...
            <div className="bg-[#0a0e17] border border-[#1a2d45] rounded-lg overflow-hidden mb-4">
              {current.context.map((row, i) => (
                ...
                >
                  <span className="text-[10px] text-[#7da0c4] font-display min-w-[80px] flex-shrink-0 pt-0.5">
                    {row.label}
                  </span>
        ...
        <div className="space-y-3">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs font-display ${
                      s.completed ? 'text-[#00ff41]' : i === currentStep ? 'text-[#e0f2fe]' : 'text-[#4a6682]'
                    }`}>
                      {s.title}
                    </span>
                  </div>
                  <span className={`text-[9px] px-1.5 py-0.5 rounded ${
                    s.completed ? 'bg-[rgba(0,255,65,0.15)] text-[#00ff41]'
                      : i === currentStep ? 'bg-[rgba(0,212,255,0.15)] text-[#00d4ff]'
          ...
          <div className="bg-[#0d1526] border border-[#1a2d45] rounded-xl p-4 text-center">
            <h4 className="text-caption text-[#7da0c4] font-display mb-2">COMPLIANCE SCORE</h4>
