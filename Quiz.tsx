...
const STORAGE_KEY_PROGRESS = 'trygit-quiz-progress';
const STORAGE_KEY_HISTORY = 'trygit-quiz-history';
...
function saveProgress(answers: AnswerRecord[], mode: QuizMode) {
  try {
    localStorage.setItem(STORAGE_KEY_PROGRESS, JSON.stringify({ answers, mode, timestamp: Date.now() }));
  } catch { /* ignore */ }
}

function clearProgress() {
  try { localStorage.removeItem(STORAGE_KEY_PROGRESS); } catch { /* ignore */ }
}

function saveToHistory(result: { score: number; total: number; mode: QuizMode; domains: Record<string, { correct: number; total: number }>; timeTaken?: number }) {
  try {
...
export default function Quiz() {
  ...
  const startQuiz = useCallback(() => {
    ...
    // For simulator mode, always use 90 questions
    const count = mode === 'simulator' ? SIMULATOR_QUESTIONS : (questionCount !== 727 ? questionCount : pool.length);
  ...
  const endQuiz = useCallback(() => {
    ...
    clearProgress();
  }, [score, questions.length, mode, domainBreakdown, timeRemaining]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    saveProgress(newAnswers, mode);
  }, [selectedOption, currentQuestion, hasSubmitted, answers, currentIndex, currentAnswer, mode]);
    ...
    return (
      <div className="min-h-[100dvh] flex items-center justify-center">
        <div className="text-center">
          ...
          />
          <p className="text-body-sm text-[#7da0c4]">Loading Exam Database...</p>
        </div>
      </div>
    );
  }
  ...
  // -- Phase: SETUP --
  if (phase === 'setup') {
    return <QuizSetup db={db!} onStart={startQuiz} mode={mode} setMode={setMode} questionCount={questionCount} setQuestionCount={setQuestionCount} selectedDomains={selectedDomains} setSelectedDomains={setSelectedDomains} includePBQs={includePBQs} setIncludePBQs={setIncludePBQs} />;
  }
    ...
    if (mode === 'simulator') {
      return (
        <SimulatorScoreReport
          ...
          questions={questions}
          timeTaken={timeTaken}
          onRetry={() => { setPhase('setup'); clearProgress(); }}
        />
      );
    }
    return (
...
function QuizSetup({
  ...
  const countOptions = [10, 25, 50, 90, 727];
  const countLabels = ['10', '25', '50', '90', 'All (727)'];
  ...
  return (
    <div className="min-h-[100dvh] bg-[#0a0e17] px-6 py-12">
      ...
      >
        <h1 className="text-h1 text-[#00ff41] mb-3 font-display">Quiz Lab</h1>
        <p className="text-body text-[#7da0c4]">
          Master the {db.metadata.total_questions} exam questions with practice mode, timed exams, and 80/20 hints.
        </p>
      </motion.div>
